...
# backups because ... backups
# echo "Great, thanks for that info. Now we'll perform a backup of the app files and database(s)..."

#backup=backup-$(date +%Y%m%d).tgz #create a timestamp name for our backup file

tar -cvzf backup-$(date +%Y%m%d).tgz $webroot # create a backup of the application files, serialize it and store it in a specific directory
# create a backup of the application database(s), serialize it and store it in the same directory as the files backup
 
# tar -czf $backUp /Users/akell/Documents/Bash/* # tar command is tar -czf name_of_archive.tar.gz /location/to/archive/
