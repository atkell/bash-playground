...
[1] (Completed) Create a backup of the Magento 2 web root and database
[2] WIP: Use Composer to update the contents of the eBizMarts mc-magento2 package
[3] WIP: Use the Magento 2 CLI to run the setup:upgrade, setup:di:compile and cache:flush commands
"
...
echo "In order to use composer, we must first change ownership to our system user"
chown -R freddie:freddie /var/www/magento2

echo "Now we may use composer to update the specific package mailchimp/mc-magento2"
composer update mailchimp/mc-magento2

echo "And now we should assign ownership of these files back to Apache"
